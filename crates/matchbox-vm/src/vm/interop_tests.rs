...
mod tests {
    ...
    fn test_bx_to_js_struct() {
        ...
        // Verify key access
        assert_eq!(
            js_sys::Reflect::get(&js, &"key1".into())
                .unwrap()
                .as_f64()
                .unwrap(),
            10.0
        ...
        assert_eq!(
            js_sys::Reflect::get(&js, &"key2".into())
                .unwrap()
                .as_bool()
                .unwrap(),
            true
    ...
    fn test_call_js_function_from_bx() {
        ...
        let result = vm
            .call_function_value(bx_func, args, None)
            .expect("call_function_value failed");
    ...
    fn test_bx_function_to_js_callback() {
        ...
        let result: JsValue =
            js_sys::Reflect::apply(&func, &JsValue::UNDEFINED, &args).expect("JS call failed");
    ...
    fn test_js_promise_is_preserved_as_js_value_until_get() {
        ...
        let result = vm
            .call_function_value(bx_func, vec![], None)
            .expect("call_function_value failed");
    ...
    fn test_js_callback_future_is_marshalled_to_promise() {
        ...
        let bx_func = BxValue::new_ptr(
            vm.heap
                .alloc(GcObject::NativeFunction(|vm, _args| Ok(vm.future_new()))),
        );
        vm.callback_registry
            .borrow_mut()
            .insert(callback_id, bx_func);
    ...
    fn test_call_non_function_js_value_fails() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("not a callable JS function")
        );
    }
    ...
    fn test_host_object_classList_behavior() {
        ...
        // Add a class via JS
        let add_fn = Reflect::get(&cl_js, &"add".into())
            .unwrap()
    ...
    fn test_host_object_event_behavior() {
        let mut vm = VM::new();
        // Create event via JS
        let event_ctor = Reflect::get(&js_sys::global(), &"Event".into())
            .unwrap()
            .dyn_into::<js_sys::Function>()
            .unwrap();
        ...
        if let GcObject::JsValue(js) = vm.heap.get(id) {
            ...
            assert_eq!(
                Reflect::get(js, &type_prop).unwrap().as_string().unwrap(),
                "click"
            );
        }
    }
        ...
        let create_proxy = Closure::wrap(Box::new(move |ptr: usize, id: u32| {
            ...
            let get_trap = Closure::wrap(Box::new(
                move |target: JsValue, prop: JsValue, _receiver: JsValue| {
                    let ptr = Reflect::get(&target, &"__matchbox_vm_ptr".into())
                        .unwrap()
                        .as_f64()
                        .unwrap() as usize;
                    let id = Reflect::get(&target, &"__matchbox_gc_id".into())
                        .unwrap()
                        .as_f64()
                        .unwrap() as u32;
